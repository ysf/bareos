...
static void FreeSavedResources(BareosResource** table)
{
  int num = my_config->r_num_;
  for (int j = 0; j < num; j++) { my_config->FreeResourceCb_(table[j], j); }
  free(table);
}
...
bool DoReloadConfig()
{
  ...
  prev_config = my_config->CopyResourceTable();
  ...
  if (ok && CheckResources() && CheckCatalog(UPDATE_CATALOG)
    ...
    BareosResource** temp_config = my_config->CopyResourceTable();

    int num_rcodes = my_config->r_num_;
    for (int i = 0; i < num_rcodes; i++) {
      // restore original config
      my_config->res_head_[i] = prev_config[i];
    }
